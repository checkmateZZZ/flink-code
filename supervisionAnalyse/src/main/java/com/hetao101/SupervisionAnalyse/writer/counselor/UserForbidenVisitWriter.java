 ...
 * @date 2019/10/14
 */
public class UserForbidenVisitWriter extends RichSinkFunction<ArrayList<Row>> {
    private static Logger logger = Logger.getLogger(UserForbidenVisitWriter.class);
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        writePs = writeConn.prepareStatement(
                "insert into htbc_app.app_supervision_course_user_learning_stat_rt " +
                        "(class_id,user_id,is_forbiden_return_visit,load_time) " +
                        "values (?,?,?,?) ON DUPLICATE KEY UPDATE " +
                        "is_forbiden_return_visit=values(is_forbiden_return_visit)," +
    ...
    public void invoke(ArrayList<Row> value, Context context) {
        try {
            for (Row row : value) {
                ...
                writePs.setString(4, (String) row.getField(3));
                //执行插入
