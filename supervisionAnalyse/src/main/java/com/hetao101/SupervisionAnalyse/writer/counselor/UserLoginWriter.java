 ...
 * @date 2019/10/14
 */
public class UserLoginWriter extends RichSinkFunction<ArrayList<Row>> {
    private static Logger logger = Logger.getLogger(UserLoginWriter.class);
    ...
    public void invoke(ArrayList<Row> value, Context context) {
        try {
            for (Row row : value) {
                ...
                writePs.setString(3, (String) row.getField(2));
                //执行插入
